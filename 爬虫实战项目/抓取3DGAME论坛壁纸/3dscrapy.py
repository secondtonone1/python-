...
#https://www.3dmgame.com/tu/3729849.html
#https://www.3dmgame.com/tu/3711642.html
...
class DownloadFrame(BasicFrame):
    ...
    def __init__(self, root=None, **args):
        ...
        row = 0
        tk.Label(self.root,  text='----https://www.3dmgame.com/tu/----').grid(row=row,column=1)
        row += 1
        tk.Label(self.root, text='----请输入要下载的地址---').grid(row=row, column=1)
        row += 1
...
if __name__ == '__main__':
    lock = threading.Lock()
    root = tk.Tk()
    root.title("抓取3DMGAME游戏壁纸 -- mooncoder")
